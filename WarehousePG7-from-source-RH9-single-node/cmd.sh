...
then
    ...
    gpstart -a -d ${COORDINATOR_DATA_DIR}/whpgsne-1/
...
before_sha256=`sha256sum ${COORDINATOR_DATA_DIR}/whpgsne-1/pg_hba.conf`
HBALINE="host     all         all             0.0.0.0/0       trust"
HBAFILE="${COORDINATOR_DATA_DIR}/whpgsne-1/pg_hba.conf"
if ! grep -Fq "${HBALINE}" "${HBAFILE}"; then
    echo "Enabling access in pg_hba.conf"
    sed -i '/# replication privilege./a\host     all         all             0.0.0.0\/0       trust' ${COORDINATOR_DATA_DIR}/whpgsne-1/pg_hba.conf
fi
after_sha256=`sha256sum ${COORDINATOR_DATA_DIR}/whpgsne-1/pg_hba.conf`

if [ "${before_sha256}" != "${after_sha256}" ];
then
    # reload the database, to apply pg_hba.conf changes
    echo "Reloading WarehousePG Database ..."
    #gpstop -M fast -a -d ${COORDINATOR_DATA_DIR}/whpgsne-1/
    #gpstart -a -d ${COORDINATOR_DATA_DIR}/whpgsne-1/
    gpstop -u -d ${COORDINATOR_DATA_DIR}/whpgsne-1/
